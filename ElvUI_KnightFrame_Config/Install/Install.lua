...
local KF_Config = E:GetModule('KnightFrame_Config')
	...
	local function InstallWindow_Close()
		...
		if E.db.KnightFrame and E.db.KnightFrame.Install_Complete and E.db.KnightFrame.Install_Complete ~= KF.Version then
			...
			do -- Window : Prev Button (Bottom Panel)
				...
				KnightFrame_InstallWindow.PrevButton:SetScript('OnClick', function(self)
					if CURRENT_PAGE ~= 1 then
						...
						PlaySoundFile('Interface\\AddOns\\ElvUI_KnightFrame\\Media\\SoundEffects\\button.ogg')
					end
				end)
	...
	function KF_Config:InstallWindow_SetPage(Page)
		...
		if Page == 1 then
			KnightFrame_InstallWindow.SubTitle:SetText('Welcome to ElvUI - |cff1784d1Knight Frame|r |cffffdc3cv'..KF.Version)
		...
		elseif Page == 2 then
			...
			if KF_Config.SelectLayout == 'Moonlight' then
				...
				KnightFrame_InstallWindow.LayoutImage2:SetTexture('Interface\\AddOns\\ElvUI_KnightFrame_Config\\Media\\Graphics\\kimsungjaedisabled.tga')
			...
			elseif KF_Config.SelectLayout == 'Kimsungjae' then
				...
				KnightFrame_InstallWindow.LayoutImage1:SetTexture('Interface\\AddOns\\ElvUI_KnightFrame_Config\\Media\\Graphics\\moonlightdisabled.tga')
	...
	function KF_Config:Install_KnightFrame()
		if KF.UIParent then KF:UnregisterEventList('PLAYER_LOGOUT', 'KnightFrame_SaveDB') end
		
		E.db.KnightFrame = {
			['Enable'] = true,
			['Installed_UI_Layout'] = KF_Config.SelectLayout,
			['Install_Complete'] = KF.Version,
		}
