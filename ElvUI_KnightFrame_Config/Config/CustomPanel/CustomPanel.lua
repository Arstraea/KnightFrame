...
KF_Config.Options.args.CustomPanel = {
	...
	order = 100,
	disabled = function() return KF.db.Enable == false or KF.db.Modules.CustomPanel.Enable == false end,
	args = {
		Enable = {
			type = 'toggle',
		...
		SelectPanel = {
			...
			set = function(_, value)
				...
				PanelInfo = E:CopyTable({}, KF_CustomPanel_Default)
				
				if value ~= '0' then
					KF:Delete_CustomPanel(0)
					KF.Update.CustomPanel_CreateNewPanel = nil
			...
			set = function(_, value)
				if value ~= '' and value ~= nil then
					if _G[value] then
						...
						KF.Update.CustomPanel_CreateNewPanel = {
							['Condition'] = true,
							['Delay'] = 0,
							['Action'] = function()
								if (not ACD.OpenFrames['ElvUI'] or not ACD.OpenFrames['ElvUI']:IsShown()) and KF.Frame[0]:IsShown() then
									KF.Frame[0]:Hide()
								elseif ACD.OpenFrames['ElvUI'] and not KF.Frame[0]:IsShown() then
									KF.Frame[0]:Show()
								end
							end,
						}
