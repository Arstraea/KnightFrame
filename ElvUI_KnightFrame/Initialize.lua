...
if KF.Update and KF.Arstraea[E.myname..'-'..E.myrealm] then
	KF.Update.CheckArstraea = nil
	...
	KF:RegisterEventList('CHAT_MSG_ADDON', function(event, prefix, message, distributionType, sender)
		if prefix == 'KnightFrame_CA' then
			...
			if usingAddOn then
				print(L['KF']..' : |cff2eb7e4'..strsplit('-', sender)..'|r 님은 '..KF:Color_Value(usingAddOn)..(usingVersion == KF.Version and ' |cffceff00('..usingVersion..')|r' or ' |cffff0000(구버전'..(usingVersion and ' : '..usingVersion..' ' or '')..')|r')..' 사용자 입니다.')
			end
			...
			for friendIndex = 1, GetNumFriends() do
				userName = GetFriendInfo(friendIndex)
				if sender:find(userName) ~= nil then return end
			end
			
			if IsInGuild() then GuildRoster() end
			for guildIndex = 1, GetNumGuildMembers(true) do
				userName = GetGuildRosterInfo(guildIndex)
				if sender:find(userName) ~= nil then return end
			end
			
			for bnIndex = 1, BNGetNumFriends() do
				_, _, _, _, userName = BNGetFriendInfo(bnIndex)
				if userName and sender:find(userName) then return end
			end
			
			userName, userRealm = strsplit('-', sender)
			userRealm = userRealm ~= '' and userRealm or E.myrealm
			
			SendChatMessage('안녕하세요, '..userName..' 님. '..usingAddOn..' 제작자인 '..E.myname..' 입니다. (__) 사용해 주셔서 감사합니다~!', 'WHISPER', nil, userRealm..(userRealm ~= E.myrealm and '-'..userRealm or ''))
		end
	end)
end
