...
elseif KF.UIParent then
	...
	KF.ExpRepDisplay_Activate = false
	...
	function KF:ExpRepDisplay_EmbedToPanel()
		if KF.ExpRepDisplay_Activate and ReputationBarMover and ExperienceBarMover and KF_ExpRepSensor then
			if not ((KF_ExpTooltip and KF_ExpTooltip:IsShown()) or (KF_RepTooltip and KF_RepTooltip:IsShown())) then
				...
				if ElvUI_ExperienceBar:IsShown() and ElvUI_ReputationBar:IsShown() then
					...
					ElvUI_ExperienceBar:Point('BOTTOMRIGHT', KF_ExpRepSensor, -2, PANEL_HEIGHT/2 + 0.5)
					ElvUI_ReputationBar:Point('TOPLEFT', KF_ExpRepSensor, 'BOTTOMLEFT', 2, PANEL_HEIGHT/2 - 0.5)
	...
	function M:UpdateExpRepDimensions()
		if not KF.ExpRepDisplay_Activate then
	...
	function M:UpdateExperience(event)
		...
		if KF.ExpRepDisplay_Activate and KF_ExpTooltip then
	...
	function M:UpdateReputation(event)
		...
		if KF.ExpRepDisplay_Activate and KF_RepTooltip then
	...
	KF.Modules['ExpRepDisplay'] = function(RemoveOrder)
		if not RemoveOrder and KF.db.Enable ~= false and KF.db.Modules.Enable ~= false and KF.db.Modules.ExpRepDisplay.Enable ~= false and KF.db.Modules.ExpRepDisplay.EmbedPanel then
			...
			if Panel and KF.db.Modules.ExpRepDisplay.EmbedLocation and (KF.db.Modules.ExpRepDisplay.EmbedLocation == 'Tab' and IsTabEnabled or IsDPEnabled) then
				...
				KF.ExpRepDisplay_Activate = true
		...
		if KF.ExpRepDisplay_Activate then
			KF.ExpRepDisplay_Activate = false
