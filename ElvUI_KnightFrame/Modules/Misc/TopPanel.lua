...
-- Last Code Checking Date		: 2014. 2. 8
...
elseif KF.UIParent then
	...
	local function UpdateLocation()
		if not (KF.db.Modules.TargetAuraTracker ~= false and UnitExists('target'))then
	...
	KF.Modules['TopPanel'] = function(RemoveOrder)
		if not RemoveOrder and KF.db.Enable ~= false and KF.db.Modules.TopPanel.Enable ~= false then
			...
			local x, y
			KF.Update.TopPanel_UpdateLocation = {
				['Condition'] = function() return not (KF.db.Modules.SynergyTracker ~= false and UnitExists('target')) and (KF_TopPanel.LocationName.text:GetText() == nil or GetUnitSpeed('player') > 0) and true or false end,
				['Delay'] = 0.25,
				['Action'] = function()
		...
		elseif KF_TopPanel then
			KF.Update.TopPanel_UpdateLocation = nil
