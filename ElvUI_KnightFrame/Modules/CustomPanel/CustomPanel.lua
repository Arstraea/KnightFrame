...
function KF:CustomPanel_Create(PanelName, PanelInfo)
	...
	if not Panel then
		...
		E.CreatedMovers[Panel.mover.name].point = E:HasMoverBeenMoved(PanelName) and E.db.movers[PanelName] or PanelInfo.Location or 'CENTER'..Info.MoverDelimiter..'ElvUIParent'
		Panel.mover:ClearAllPoints()
		Panel.mover:SetPoint(unpack({string.split(Info.MoverDelimiter, E.CreatedMovers[Panel.mover.name].point)}))
...
function KF:CustomPanel_Button_Create(ButtonFrame, ButtonType, ButtonTransparency)
	...
	if not ButtonFrame then
		if #DeletedButton > 0 then
			...
			KF:TextSetting(Button, nil, { FontStyle = 'OUTLINE' }, 'CENTER', 0, 0)
