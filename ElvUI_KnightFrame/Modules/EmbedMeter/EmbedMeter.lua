...
-- Last Code Checking Date		: 2014. 2. 21
...
--------------------------------------------------------------------------------
	...
	if SkadaLoaded then
		...
		hooksecurefunc(Skada, 'ApplySettings', function() if Activate then KF:EmbedMeter_EmbedSetting_Skada() end end)
		hooksecurefunc(Skada, 'DeleteWindow', function() if Activate then KF.Modules.EmbedMeter() end end)
	end
	...
	if RecoundLoaded then
		...
		Recount_MainWindow:HookScript('OnMouseDown', function(self)
			...
			if Activate and Recount_MainWindow:GetParent() ~= UIParent and Recount.db.profile.KnightFrame_Embed then
				...
				for _, direction in pairs({ 'LEFT', 'RIGHT', }) do
					...
					if EmbedAddOn == 'Skada' then
						...
						if width and height then
							...
							window.bargroup.RegisterCallback(Skada.displays.bar, 'AnchorMoved', function(cbk, group, x, y)
								if Activate and window.bargroup:GetParent() ~= UIParent and window.db.KnightFrame_Embed then
									window.bargroup:SetParent(UIParent)
									window.db.KnightFrame_Embed = nil
	...
	KF.Modules['EmbedMeter'] = function(RemoveOrder)
		...
		if not RemoveOrder and KF.db.Enable ~= false and KF.db.Modules.EmbedMeter.Enale ~= false then
			Activate = true
