...
elseif KF.UIParent then
	...
	KI.UnitPopupList = { ['FRIEND'] = true, ['GUILD'] = true, ['RAID'] = true, ['FOCUS'] = true, ['PLAYER'] = true, ['PARTY'] = true, ['RAID_PLAYER'] = true }
	KI.ModelList = {
	...
	function KI:CreateInspectFrame()
		...
		do --<< Specialization Page >>--
			...
			for i = 1, NUM_GLYPH_SLOTS do
				...
				self.Spec['Glyph'..i]:SetFrameLevel(CORE_FRAME_LEVEL + 2)
				...
				self.Spec['Glyph'..i].Icon:SetFrameLevel(CORE_FRAME_LEVEL + 3)
				...
				self.Spec['Glyph'..i].Tooltip:SetFrameLevel(CORE_FRAME_LEVEL + 4)
		...
		do --<< UnitPopup Setting >>--
			...
			hooksecurefunc('UnitPopup_ShowMenu', function(Menu, Type, Unit, Name)
				if KI.Activate and UIDROPDOWNMENU_MENU_LEVEL == 1 and KI.UnitPopupList[Type] then
					local Button
					local DataTable = {
	...
	KI.INSPECT_HONOR_UPDATE = function(Event)
		if Event or HasInspectHonorData() then
			for i, Type in pairs({ '2vs2', '3vs3', '5vs5' }) do
				KI.CurrentInspectData.PvP[Type] = { GetInspectArenaData(i) }
				for i = 4, #KI.CurrentInspectData.PvP[Type] do
					KI.CurrentInspectData.PvP[Type][i] = nil
				end
			end
			KI.CurrentInspectData.PvP.RB = { GetInspectRatedBGData() }
			KI.CurrentInspectData.PvP.Honor = select(5, GetInspectHonorData())
		end
		
		if not KI.ForbidUpdatePvPInformation then
			KI:InspectFrame_PvPSetting(KI.CurrentInspectData)
		end
	end
	...
	function KI:InspectFrame_PvPSetting(DataTable)
